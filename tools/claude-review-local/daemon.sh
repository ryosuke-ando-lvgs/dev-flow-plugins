...
poll_issue_fill() {
  ...
  for allowed in $CLAUDE_REVIEW_ALLOWED_USERS; do
    local events count i
    # gh api の --jq はシェル変数展開のみで --arg 相当の機能が無いため、
    # 生イベントを一旦そのまま取得してから、別プロセスの jq --arg でトリガー文字列を渡す
    # （$CLAUDE_REVIEW_FILL_TRIGGER に " 等が含まれても壊れないようにするため）。
    events="$(gh api "users/$allowed/events" -X GET --paginate --jq '.' 2>/dev/null \
      | jq -s 'add // []' \
      | jq --arg trigger "$CLAUDE_REVIEW_FILL_TRIGGER" \
        '[.[] | select(.type=="IssueCommentEvent" and .payload.issue.pull_request == null and (.payload.comment.body | sub("^[\r\n]+"; "") | startswith($trigger))) | {repo: .repo.name, number: .payload.issue.number, comment_id: .payload.comment.id, created_at: .payload.comment.created_at, author: .actor.login}]' 2>/dev/null || echo '[]')"
    ...
    while [ "$i" -lt "$count" ]; do
      ...
      if ! crl_is_allowed_user "$author"; then
        crl_mark_processed "$GLOBAL_STATE_FILE" "fill-$comment_id" "$created_at" "fill_last_seen"
        continue
      fi
